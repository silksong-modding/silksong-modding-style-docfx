...
if (process.argv.some(x => x === "--clean"))
	clean();
else
	build();
...
function build(): void {
	const
		isTestBuild = process.argv.some(x => x === "--test"),
	...
	for (const file of fs.readdirSync(srcRoot, { recursive: true, withFileTypes: true })) {
		...
		const
			isScss = extLower === ".scss",
			isJs = extLower === ".js",
			shouldCompileScss = !file.name.startsWith("_"), // don't compile partials
			shouldMinifyJs = path.basename(file.name, ext) !== "main" && splitDir.some(x => x === "public"),
		...
		try {
			if (isScss && shouldCompileScss)
				compileScss(file, destDir, isTestBuild);
			else if (isJs && shouldMinifyJs)
				minifyJs(file, destDir, isTestBuild);
		} catch (e) {
	...
	function compileScss(file: fs.Dirent<string>, destDir: string, sourceMap: boolean): void {
		...
		const
			result = sass.compile(fullPath(file), { sourceMap, style: "compressed" } ),
		...
		if (result.sourceMap) {
			...
			result.sourceMap.sources.forEach((s, i, arr) =>
	...
	function minifyJs(file: fs.Dirent<string>, destDir: string, sourceMap: boolean): void {
		...
		const
			jsName = `${path.basename(file.name, path.extname(file.name))}.min.js`,
