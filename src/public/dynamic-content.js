...
export async function initNav() {
  const mainJs = import("./main.js").then((m) => m.default),
  ...
  async function buildNavLinks() {
    const itemTmpl = document.body.querySelector("#nav-item-tmpl"),
      dropdownTmpl = document.body.querySelector("#nav-item-dropdown-tmpl");

    const navUrl = new URL(meta("docfx:navrel").replace(/.html$/gi, ".json"), window.location.href),
    ...
    for (const link of links) {
      ...
      if (ul instanceof HTMLElement) {
        ...
        for (const x of ul.querySelectorAll("li")) x.classList = "nav-item nav-link dropdown-item";
      }
    }
    return links;
  }
  ...
  async function buildIconLinks() {
    const iconLinks = (await mainJs).iconLinks,
      iconChildren = [];
    ...
    onFontsLoaded((fonts) => {
      if (fonts.findIndex((x) => x.family === "bootstrap-icons") !== -1) return;
  ...
  async function setupThemePicker() {
    ...
    function setTheme(theme) {
      ...
      if (theme === "auto")
        document.documentElement.dataset.bsTheme = window.matchMedia("(prefers-color-scheme: dark)")
          .matches
          ? "dark"
          : "light";
      else document.documentElement.dataset.bsTheme = theme;
  ...
  function addAutoCloseEvent(details) {
    details.addEventListener("focusout", (e) => {
      if (details.open && !details.contains(e.relatedTarget)) details.removeAttribute("open");
    });
  }
}

export async function initToc() {
  const tocEl = document.body.querySelector("#toc-offcanvas ul"),
  ...
  const locSearchResults = loc("searchResultsCount"),
  ...
  if (!disableTocFilter) {
    ...
    function filterChanged() {
      if (filterEl.value === tocFilter) return;
      ...
      const treeWalker = document.createTreeWalker(tocEl, NodeFilter.SHOW_ELEMENT, (node) =>
        node.tagName === "LI" &&
        !node.textContent.toLowerCase().includes(filterLower) &&
        ![...node.querySelectorAll("li, a, summary")].some((child) =>
          child.textContent.toLowerCase().includes(filterLower),
        )
      ...
      if (count <= 0) announceEl.classList.remove("sr-only");
    }
  }
...
export async function initAffix() {
  ...
  const affixEl = document.body.querySelector("#affix-custom ul"),
...
export async function initSearch() {
  ...
  const containerEl = document.querySelector("#search-results-container"),
  ...
  const worker = new Worker(
    new URL(`${relHref}./public/search-worker.min.js`, window.location.href),
    { type: "module" },
  );
    ...
    switch (event.data.e) {
      case "index-ready":
        ...
        searchForm.addEventListener("submit", (e) => {
          e.preventDefault();
          if (query !== "") location.replace("#" + containerEl.id);
        });
        window.docfx.searchReady = true;
        break;
      case "query-ready":
        if (searchBar.value === "") document.body.removeAttribute("data-search");
        else {
  ...
  function displayPage(hits, page) {
    const perPage = parseInt(meta("docfx:searchResultsPerPage")) || 10,
    ...
    if (hits.length <= 0) {
      ...
      countEl.textContent =
        loc("searchResultsCount", { query, count: hits.length }) +
    ...
    if (pages > 1) {
      const numLinks = parseInt(meta("docfx:searchNumPaginationLinks")) || 5,
      ...
      for (let i = pStart; i < pEnd; i++) {
        let li;
        if (i === page) li = cloneTemplate("search-pagination-active-tmpl", "li");
        else {
...
function cloneTemplate(id, selector) {
  if (id in templates === false) templates[id] = document.getElementById(id);
...
function buildNavTree(items, ul, baseUrl) {
  ...
  for (const item of items) {
    ...
    if ("items" in item) {
      ...
      } else summary.append(nameNode);
      buildNavTree(item.items, subUl, baseUrl);
    } else {
      li = document.createElement("li");
      if ("href" in item) li.append(buildLink(nameNode, item.href));
      else li.append(nameNode);
    }

    if (ul instanceof HTMLElement) ul.append(li);
    else ul.push(li);
  }
  return ul;
  ...
  function buildLink(textNode, href) {
    ...
    } else if (sameUrl(url, currentUrl)) a.setAttribute("aria-current", "page");
    return a;
  }
}
